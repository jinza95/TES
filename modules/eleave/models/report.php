...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params)
    {
        ...
        if (!empty($params['department'])) {
            $where[] = ['F.department', $params['department']];
        }
        if (!empty($params['member_id'])) {
            $where[] = ['F.member_id', $params['member_id']];
        }
        if (!empty($params['leave_id'])) {
            $where[] = ['F.leave_id', $params['leave_id']];
        }
        ...
        return static::createQuery()
            ...
            ->from('leave_items F')
            ->join('user U', 'LEFT', ['U.id', 'F.member_id'])
            ->where($where);
    }
        ...
        if ($request->initSession() && $request->isReferer() && $login = Login::isMember()) {
            ...
            if (preg_match_all('/,?([0-9]+),?/', $request->post('id')->toString(), $match)) {
                if ($action == 'detail') {
                    ...
                    if (!empty($ids)) {
                        // ลบ database
                        $this->db()->delete($this->getTableName('leave_items'), ['id', $ids], 0);
                        // log
